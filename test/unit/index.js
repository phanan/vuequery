...
describe('VueQuery', () => {
  ...
  describe('closest()', () => {
    it('finds closest to be self', () => {
      expect($vm.find('qux')[0].closest('qux').is('qux')).to.be.true
    })

    it('finds closest to be parent', () => {
      expect($vm.find('foo')[0].closest('bar').is('bar')).to.be.true
    })

    it('finds closest to be ancestor', () => {
      expect($vm.find('foo')[0].closest('qux').is('qux')).to.be.true
    })
  ...
  describe('next()', () => {
    it('finds next without name', () => {
      expect($vm.find('baz')[0].next().is('noop')).to.be.true
    })

    it('finds next with name', () => {
      expect($vm.find('baz')[0].next('noop').is('noop')).to.be.true
    })
  ...
  describe('parent()', () => {
    ...
    it('finds parent with name specified', () => {
      expect($vm.find('baz')[0].parent('qux').is('qux')).to.be.true
    })
  ...
  describe('prev()', () => {
    it('finds prev without name', () => {
      expect($vm.find('baz')[1].prev().is('noop')).to.be.true
    })

    it('finds prev with name', () => {
      expect($vm.find('baz')[1].prev('noop').is('noop')).to.be.true
    })
